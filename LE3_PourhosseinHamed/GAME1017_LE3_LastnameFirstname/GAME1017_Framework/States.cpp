...
void State::Update()
{
	...
	{
		i.second->Update();  // this is the update() method of the derived GameObject.
...
void GameState::Update()
{
	...
	AsteroidField* fieldObject = static_cast<AsteroidField*> (GetChild("field"));
	vector<Asteroid*>& field = fieldObject->GetAsteroids();
	// Collision of ship and asteroids. (asteroid vector)
	if (ship != nullptr) // ship still exists so do collision with it.
	{
	   for (unsigned i = 0 ; i<field.size();i++)
	   {
		   Asteroid* ast = field.at(i); // same as field[i]
		if(COMA::CircleCircleCheck(ship->GetCenter(),ast->GetCenter(),ship->GetRadius(),ast->GetRadius()))
		{
	...
	//if (field.size() > 0)
 //               {
	//	 float bulletAngle = atan2(bul->GetDst()->y, bul->GetDst()->x);
	//	   float leftAngle = bulletAngle - (M_PI / 4.0f); // -45 degrees
 //         float rightAngle = bulletAngle + (M_PI / 4.0f); // +45 degrees
 //                   field.push_back(new Asteroid({ 539, 0, 61, 66 },
 //                       { ast->GetDst()->x  , ast->GetDst()->y ,
	//					40.26f, 45.26f }));
 //                   field.back()->SetColMods((rand() % 129), (rand() % 129), (rand() % 129));
 ...
 //               }
				short size = ast->GetSize();
				if (size > 0) {
					--size;
					for (int j = 0; j < 2; ++j)
					{
						auto SpawnChunk = new Asteroid({ 539, 0, 61, 66 },
							{ ast->GetDst()->x,
							 ast->GetDst()->y,
							 ast->GetDst()->w - 20, ast->GetDst()->h - 20 });
						auto color = ast->GetColMods();
						if (j == 0)
						{
							SpawnChunk->UpdateDeltas(bul->GetAngle() + ((rand() % 15) + 30));
						}
						else
						{
							SpawnChunk->UpdateDeltas(bul->GetAngle() - ((rand() % 15) + 30));
						}
						SpawnChunk->SetColMods(color[0], color[1], color[2]);
						SpawnChunk->SetSize(size);
						field.push_back(SpawnChunk);
					}
				            }
